...
export const connectWithWebSocket = () => {
  ...
  socket.on('group-call-limit-reached', (data) => {
    const { roomId } = data;
    console.log(`Group call limit reached in room ${roomId}`);
    alert(`Group call limit reached in room ${roomId}. You cannot join.`);
  });

  socket.on('room-id-result', (data) => {
    console.log('Room id result: ', data.result);
    if (data.result === 'room-id-invalid') {
      webRTCGroupCallHandler.createNewGroupCall(data.roomId);
    }
    else if(data.result === 'room-id-exist'){
      webRTCGroupCallHandler.joinGroupCall(socket.id ,data.roomId);
    }
  });
...
export const checkRoomId = (roomId) => {
  ...
  socket.emit('check-room-id', roomId);
}
...
const handleBroadcastEvents = (data) => {
  switch (data.event) {
    ...
    case broadcastEventTypes.GROUP_CALL_ROOMS:
      ...
      // const roomToIDs = data.roomToIDs;

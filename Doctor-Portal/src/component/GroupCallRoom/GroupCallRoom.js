...
const toggleRecording = () => {
  ...
  return (
    <div className='group_call_room_container'>
      ...
      {!callended && <p style={{color:"white"}}>{formatTime(elapsedTime)}</p>}
      {callended && <p style={{ color: "white" }}>Call Ended</p>}
      </div>
      ...
      <div className="trio1">
      {rcdconsent && <button  color="primary" aria-label="record" onClick={toggleRecording}>
                            {isrd ? <FaRegStopCircle /> : <BsRecordCircle />}
                        </button>}
      {<button  onClick={handleDownloadRecording}><MdDownloading /></button>}
